...
class MemberAreaPage extends Component<Props> {
    render() {
        return (
          ...
          {this.props.role && [roles.PARTICIPANT_ROLE, roles.STARTUP_ROLE, roles.ADMIN_ROLE, roles.SUPERVISOR_ROLE, roles.MENTOR_ROLE].includes(this.props.role) && <Redirect to='/private/member-area' />}
          </div>
        )
    }
}
